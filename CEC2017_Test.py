...
def run():
    ...
    print("LSHADE-RSP started on CEC2017 F1...")

    optimizer = LSHADE_RSP(
        func=objective,
        bounds=bounds,
        pop_size=pop_size,
        max_evals=max_evals,
...
def plot_convergence(fes, gap):
    ...
    plt.plot(fes, gap, lw=2, label="LSHADE-RSP")

    plt.yscale("log")
    plt.xlabel("Function Evaluations")
    plt.ylabel("Fitness Gap (log scale)")
    plt.title("LSHADE-RSP Performance on CEC2017 F1 (D=30)")
